...
public class UserService {
    ...
    public List<User> getNUser(int n){
        ...
        List<User> users = repository.findNUser(limitUser);
        return users;
    }

    public List<User> getUsersWithAgeSup(int age, int limit){
        Pageable limitUser = PageRequest.of(0,limit);
        Date date = new Date();
        Calendar cal = Calendar.getInstance();
        cal.add(Calendar.YEAR, -age);
        date = cal.getTime();
        List<User> users = repository.findWithAgeSup(date, limitUser);
        return users;
    }

    public List<User> getUsersWithAgeEq(int age, int limit){
        Pageable limitUser = PageRequest.of(0,limit);
        Date date;
        Date dateMax;
        Calendar cal = Calendar.getInstance();
        ...
        List<User> users = repository.findWithAgeEq(date,dateMax,limitUser);
        return users;
    }

    public List<User> getAllUsersByName(String name , int limit){
        Pageable limitUser = PageRequest.of(0,limit);
        List<User> users = repository.findWithName(name,limitUser);
        return users;
    }
